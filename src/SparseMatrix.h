...
typedef VF VPROFILE;
typedef VVF VVPROFILE;
const PROFILE_TYPE PROFILE_ZERO = 0.0f;
const PROFILE_TYPE PROFILE_ONE = 1.0f;
const PROFILE_TYPE PROFILE_MAX = FLT_MAX;

const PROFILE_TYPE POSTERIOR_CUTOFF = 0.01;         // minimum posterior probability
                                             // value that is maintained in the
                                             // sparse matrix representation

typedef pair<int,PROFILE_TYPE> PID;                 // Sparse matrix entry type
                                             //   first --> column
                                             //   second --> value
...
class SparseMatrix {
  ...
  SafeVector<PID> data;                           // data values
  SafeVector<SafeVector<PID>::iterator> rowPtrs;  // pointers to the beginning of each row
 ...
 public:
  ...
  SparseMatrix (int seq1Length, int seq2Length, const VPROFILE &posterior) :
    ...
    VPROFILE::const_iterator postPtr = posterior.begin();
    ...
    SafeVector<PID>::iterator dataPtr = data.begin();
  ...
  SafeVector<PID>::iterator GetRowPtr (int row) const {
  ...
  PROFILE_TYPE GetValue (int row, int col){
    assert (row >= 1 && row <= seq1Length);
  ...
  SparseMatrix *ComputeTranspose () const {
    ...
    // now fill in data
    SafeVector<SafeVector<PID>::iterator> currPtrs = ret->rowPtrs;

    for (int i = 1; i <= seq1Length; i++){
      SafeVector<PID>::iterator row = rowPtrs[i];
      for (int j = 0; j < rowSize[i]; j++){
  ...
  VPROFILE *GetPosterior () const {

    // create a new posterior matrix
    VPROFILE *posteriorPtr = new VPROFILE((seq1Length+1) * (seq2Length+1)); assert (posteriorPtr);
    VPROFILE &posterior = *posteriorPtr;

    // build the posterior matrix
    for (int i = 0; i < (seq1Length+1) * (seq2Length+1); i++) posterior[i] = 0;
    for (int i = 1; i <= seq1Length; i++){
      VPROFILE::iterator postPtr = posterior.begin() + i * (seq2Length+1);
      for (int j = 0; j < rowSize[i]; j++){
